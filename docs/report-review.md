...
Use `repo-context-doctor .`, `repo-context-doctor . --json`, `repo-context-doctor . --markdown --output context-report.md`, or `repo-context-doctor . --no-score` as appropriate. The scan does not modify the target repository.
...
The optional score is not a benchmark of code quality, agent success, security, or maintainability. The scanner is local, does not call an LLM or network service, never executes target commands, and does not upload reports. Review repository-relative paths and instruction excerpts before sharing output.